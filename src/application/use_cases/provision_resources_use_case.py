...
class ProvisionResourcesUseCase:
    ...
    async def execute(self, command: ProvisionResourcesCommand, skip_high_risk_check: bool = False, skip_collision_check: bool = False, user_token: str = "") -> ProvisionResourcesResponseDTO:
        ...
        blueprint = await self._generate_blueprint(command.prompt)
        ...
        try:
            created_term_sets = await self.enterprise_provisioner.provision_term_sets(blueprint)
        ...
        try:
            created_content_types = await self.enterprise_provisioner.provision_content_types(blueprint)
        except (DomainException, SharePointProvisioningException) as err:
        ...
        created_sites, site_links, site_warnings = await self.site_provisioner.provision(blueprint)
            ...
            if needs_resolution:
                ...
                if target_site_id and "," not in target_site_id:
                    try:
                        ...
                        resolved = await self.sharepoint_repository.get_site(target_site_id)
                ...
                if (not resolved or not resolved.get("id")) and site_result.get("webUrl"):
                    try:
                        resolved = await self.sharepoint_repository.get_site_by_url(site_result["webUrl"])
                    except Exception as e:
                ...
                if not resolved or not resolved.get("id"):
                    try:
                        ...
                        for attempt in range(3):
                            ...
                            found = await self.sharepoint_repository.search_sites(site_result.get("displayName", ""))
        ...
        created_lists, list_links, list_warnings = await self.list_provisioner.provision(
            blueprint,
        ...
        created_pages, page_links, page_warnings = await self.page_provisioner.provision(blueprint, site_id=target_site_id)
        warnings.extend(page_warnings)
        
        created_libs, deleted_libs, lib_title_to_id, lib_links, lib_warnings = await self.library_provisioner.provision(blueprint, site_id=target_site_id)
        warnings.extend(lib_warnings)

        # 5. Provision groups and permissions
        created_groups, group_warnings = await self.group_provisioner.provision(
            blueprint,
            lib_title_to_id
        )
        ...
        created_views = await self.enterprise_provisioner.provision_views(blueprint)
        created_workflows = await self.enterprise_provisioner.scaffold_workflows(blueprint)
            ...
            if target_lib_id:
                ...
                for wf in created_workflows:
                    ...
                    try:
                        ...
                        await self.sharepoint_repository.upload_file(
    ...
    async def _generate_blueprint(self, prompt: str) -> ProvisioningBlueprint:
    ...
    def _suppress_library_permission_assignments(self, blueprint: ProvisioningBlueprint, prompt: str) -> None:
        """Remove library-targeted groups when prompt requests default/open access.

        This avoids noisy permission-assignment warnings when the user selected
        "No, everyone can access" during requirement gathering.
        """
        ...
        if not any(marker in prompt_l for marker in open_access_markers):
            return
        ...
        if removed > 0:
            logger.info("Suppressed %d library permission group assignment(s) due to open-access request", removed)
        ...
        try:
            ...
            for lib in blueprint.get_all_document_libraries():
                ...
                if lib.action == ActionType.CREATE and title_lower in existing_titles:
                    lib.action = ActionType.UPDATE
                    lib.library_id = existing_titles[title_lower]
